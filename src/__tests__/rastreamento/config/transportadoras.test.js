...
beforeAll(() => {
  const configPath = path.join(__dirname, '../../../../public/javascripts/rastreamento/config/transportadoras.js');
...
describe('RastreamentoConfig', () => {
  ...
  describe('transportadoras', () => {
    test('deve ter array de transportadoras definido', () => {
      ...
      expect(Array.isArray(window.RastreamentoConfig.transportadoras)).toBe(true);
    });

    test('deve ter pelo menos uma transportadora', () => {
      expect(window.RastreamentoConfig.transportadoras.length).toBeGreaterThan(0);
    });
    ...
    test('nomes devem ser únicos', () => {
      const nomes = window.RastreamentoConfig.transportadoras.map((t) => t.nome);
  ...
  describe('coresTransportadoras', () => {
    test('deve ter objeto de cores definido', () => {
      ...
      expect(typeof window.RastreamentoConfig.coresTransportadoras).toBe('object');
    });

    test('deve ter cores no formato rgba válido', () => {
      Object.values(window.RastreamentoConfig.coresTransportadoras).forEach((cor) => {
        expect(cor).toMatch(/^rgba\([\d\s,]+\)$/);
      });
    });
  ...
  describe('dataRastreamento', () => {
    ...
    test('deve estar no formato YYYY-MM-DD', () => {
      expect(window.RastreamentoConfig.dataRastreamento).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    });
