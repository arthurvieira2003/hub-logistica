...
beforeAll(() => {
  const formattersPath = path.join(__dirname, '../../../../public/javascripts/rastreamento/utils/formatters.js');
...
describe('RastreamentoUtils', () => {
  ...
  describe('formatarData', () => {
    test('deve formatar data de YYYY-MM-DD para DD/MM/YYYY', () => {
      expect(window.RastreamentoUtils.formatarData('2024-01-15')).toBe('15/01/2024');
      expect(window.RastreamentoUtils.formatarData('2024-12-31')).toBe('31/12/2024');
      expect(window.RastreamentoUtils.formatarData('2024-03-05')).toBe('05/03/2024');
    });

    test('deve retornar data já formatada como está', () => {
      expect(window.RastreamentoUtils.formatarData('15/01/2024')).toBe('15/01/2024');
      expect(window.RastreamentoUtils.formatarData('31/12/2024')).toBe('31/12/2024');
    });

    test('deve remover parte da hora se existir', () => {
      expect(window.RastreamentoUtils.formatarData('2024-01-15 10:30:00')).toBe('15/01/2024');
      expect(window.RastreamentoUtils.formatarData('2024-12-31 23:59:59')).toBe('31/12/2024');
    });
  ...
  describe('formatarDataHora', () => {
    test('deve formatar data e hora corretamente', () => {
      const result = window.RastreamentoUtils.formatarDataHora('2024-01-15 10:30:00');
      // formatarDataHora chama formatarData internamente
    ...
    test('deve retornar apenas data formatada se não houver hora', () => {
      ...
      // Quando não há hora, chama formatarData diretamente
  ...
  describe('calcularDiasAtraso', () => {
    test('deve calcular dias de atraso corretamente (formato YYYY-MM-DD)', () => {
      // Data de hoje: 2024-01-15, previsão: 2024-01-10 (5 dias atrás)
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('2024-01-10');
      expect(diasAtraso).toBe(5);
    });

    test('deve calcular dias de atraso corretamente (formato DD/MM/YYYY)', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('10/01/2024');
      expect(diasAtraso).toBe(5);
    });

    test('deve retornar 0 para nota não atrasada', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('2024-01-20');
      expect(diasAtraso).toBe(0);
    });

    test('deve retornar 0 para data de hoje', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('2024-01-15');
      expect(diasAtraso).toBe(0);
    });

    test('deve retornar 0 para data inválida', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('data-invalida');
      expect(diasAtraso).toBe(0);
    });

    test('deve calcular corretamente para 1 dia de atraso', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('2024-01-14');
      expect(diasAtraso).toBe(1);
    });

    test('deve calcular corretamente para 30 dias de atraso', () => {
      const diasAtraso = window.RastreamentoUtils.calcularDiasAtraso('2023-12-16');
  ...
  describe('renderizarLogoTransportadora', () => {
    test('deve renderizar ícone FontAwesome', () => {
      ...
      const html = window.RastreamentoUtils.renderizarLogoTransportadora(transportadora);
  ...
  describe('obterCorBordaTransportadora', () => {
    test('deve retornar cor específica para transportadora conhecida', () => {
      expect(window.RastreamentoUtils.obterCorBordaTransportadora('Ouro Negro')).toBe(
        'rgba(255, 204, 0, 0.3)'
      );
      expect(window.RastreamentoUtils.obterCorBordaTransportadora('Expresso Leomar LTDA')).toBe(
        'rgba(0, 52, 150, 0.3)'
      );
    });

    test('deve retornar cor padrão para transportadora desconhecida', () => {
      const cor = window.RastreamentoUtils.obterCorBordaTransportadora('Transportadora Desconhecida');
    ...
    test('deve retornar todas as cores conhecidas corretamente', () => {
      const transportadoras = [
        ...
        { nome: 'Mengue Express transportes LTDA', cor: 'rgba(255, 101, 38, 0.3)' },
        { nome: 'Transportes Expresso Santa Catarina LTDA', cor: 'rgba(2, 118, 116, 0.3)' },
        { nome: 'Expresso Princesa Dos Campos S/A', cor: 'rgba(14, 88, 46, 0.3)' },
      ];

      transportadoras.forEach(({ nome, cor }) => {
        expect(window.RastreamentoUtils.obterCorBordaTransportadora(nome)).toBe(cor);
      });
    });
  });
});
